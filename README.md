...
- root에는 **map만**, 영역별 가이드는 **그 영역 폴더 안에** 둡니다 → 작업 영역만 로드되어 **컨텍스트 토큰 절약**.
- 코드만 봐선 알 수 없는 함정·배경을 **8섹션 템플릿(WHAT / CONTENTS / HOW / HOW NOT / WHERE / WHY / COMMANDS / LEARNED CAUTIONS)** 으로 명시합니다.
- 각 가이드는 **첫 줄 미션 + Tradeoff** 로 LLM이 룰의 의도와 비용을 함께 인지하게 합니다.
- 3단계 워크플로우: **`init`(가벼운 뼈대)** → **`/learn`(주의사항 누적)** → **`/update`(인터뷰로 가이드 채우기)**.
- `/guide-audit` 명령으로 모든 가이드를 결정적 루브릭으로 채점합니다.
...
7. **COMMANDS** — 빌드/테스트/린트 + 영역 고유 명령어 가드
8. **⚠️ LEARNED CAUTIONS** — 별도 파일 `LEARNED_CAUTIONS.md`로 분리

특히 4(HOW NOT)와 6(WHY)는 **사람만 알 수 있는 지식**을 명시적으로 담는 자리입니다. 코드 추측만으로는 노이즈가 되는 영역이라 init은 **1·2·5·7만 채우고**, **3·4·6은 `/update` 인터뷰에서 사용자 결정으로** 채웁니다. 사람용 작성 안내문은 HTML 주석으로 격리되어 자동 로드 시 토큰 비용 0입니다.
...
해당 영역 폴더의 `LEARNED_CAUTIONS.md`에 한 줄 추가되고, 본문 가이드는 그 파일을 자동 import 합니다. 다음 세션부터 에이전트가 같은 실수를 안 합니다.

<br/>

### 5. /update 인터뷰 = 코드에 맞춰 가이드를 같이 키운다

베이스라인이 잡힌 뒤 `/update`를 실행하면, 영역마다 **9가지 질문 유형**(변경 사실·컨벤션·안티패턴·암묵지·드리프트 등)으로 사용자와 인터뷰합니다. 각 제안에는 **근거 한 줄**(커밋 ID, 파일 위치)이 붙어 잘 모르는 영역도 판단 가능합니다.

인터뷰 후 **영역 간 충돌 6종**(컨벤션 모순 / WHERE 중복 / HOW NOT vs HOW / 용어 불일치 / 의존 방향 / COMMANDS)을 검사하고, 미해소면 update가 **미완료**로 표시되어 다음 실행에서 그 충돌부터 다시 묻습니다.

영역 경계 재구성이 필요하면 `/update --restructure <영역>` 로 영역 자체를 다시 그릴 수 있습니다 (한 번에 한 영역, 사용자 승인 전엔 파일 이동 0건).

**사용자 결정은 자동 덮어쓰지 않습니다** — 변경 시 반드시 확인 + 이전 결정은 변경 이력 주석으로 보존.

<br/>

### 6. /guide-audit 결정적 채점 = 품질 회귀 방지
...
→ Codex만 `$` 접두사. 누적 대상은 현재 영역의 `LEARNED_CAUTIONS.md`이며 본문 가이드는 수정하지 않습니다.
...
→ Codex에서는 `$update` 형식. 베이스라인 완성 직후 첫 실행, 이후 월 1회 또는 주요 기능 추가/버그 클러스터/의존성 교체/릴리즈 직전에 갱신하세요. (대규모 리팩터링 중에는 피하고 완료 후 실행.)
...
현재 작업 영역의 `LEARNED_CAUTIONS.md`에 한 줄 추가합니다 (본문 가이드는 수정하지 않음). 영역이 모호하면 사용자에게 확인하고, 인자 없이 호출하면 최근 대화에서 잘못된 내용을 자동 추출합니다.

**Q. `/update`는 언제 실행하나요?**

베이스라인이 완성되어 패턴이 잡힐 즈음 첫 실행, 이후 주요 기능 추가·외부 의존성 교체·버그 fix 클러스터·릴리즈 직전에 갱신합니다. 각 제안에 근거(커밋 ID, 파일 위치)가 붙고, 사용자 결정은 자동 덮어쓰지 않으며 영역 간 충돌이 미해소면 update가 미완료로 표시됩니다.

**Q. `/update --restructure`는 무엇이 다른가요?**

일반 `/update`는 가이드 내용만 갱신하고, `--restructure`는 영역 자체를 분할/병합/이름 변경합니다. 파일 이동·가이드 재배치·`LEARNED_CAUTIONS` 항목 이전까지 모두 사용자 확인을 거치며, 한 번에 한 영역만 처리합니다.
