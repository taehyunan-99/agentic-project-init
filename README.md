...
- `/guide-audit` 명령으로 프로젝트 내 모든 가이드를 v1.2 루브릭으로 채점합니다.
...
7. **COMMANDS** — 빌드/테스트/린트 명령어 + 명령어 가드
8. **⚠️ LEARNED CAUTIONS** — `learn` 스킬로 누적

특히 4(HOW NOT)와 6(WHY)는 **사람만 알 수 있는 지식**을 명시적으로 담는 자리이고, 7(COMMANDS)은 LLM이 명령어를 추측하지 못하게 막는 가드입니다. 사람이 봐도 되는 작성 안내문은 HTML 주석으로 격리해 자동 로드 시 토큰 비용 0.
