...
{이 프로젝트의 핵심 의도 — 영역 라우팅 외에 LLM이 root에서 즉시 알아야 할 1문장. 카파시 원칙 #16.}
...
이 map 구조를 따르면 잃는 것이 있다면 한 줄 명시.
예: "영역 라우팅을 LLM이 판단하므로 영역 경계가 모호한 작업은 두 가이드를 모두 봐야 함 — 토큰 비용 약간 증가, 단일 거대 가이드 대비 절감 폭이 훨씬 큼."
