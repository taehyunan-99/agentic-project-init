# Agentic Project Init — 가이드 파일 품질 루브릭
...
### D2'. 8섹션 템플릿 준수 (3점) — applies_to: 2
- 만점(3): 8섹션 중 6개 이상 매칭.
- 부분(1): 4~5개. 0점: 3개 이하.
- 영어/한국어 aliases 매칭으로 자생 한국어 가이드도 정상 인정.
...
- 부분(1): 외부 링크 1개. 0점: 없음.
- 양적 매칭만 보는 D4 점수와 별개로, **"어디에 @import를 써야 하는가"는 아래 "@import 최소주의 원칙"을 따른다.** 자동 채점은 양만 본다.

---

## 부속 원칙 — @import 최소주의

`@import`는 컨텍스트에 본문을 **자동 주입**하는 강한 단언이다. 다음 비용이 있다.
...
→ **@import는 그 추상화 없이는 안 되는 케이스에만 쓴다.** 그 외는 마크다운 링크가 더 부합한다.
...
### D5. HTML 주석으로 메타 안내 격리 (3점) — applies_to: 1, 2, 3
...
### H3. CLAUDE.md ↔ AGENTS.md sync 정합성 (2점) — applies_to: 1
...
- *sync 쌍 제외*: **같은 디렉토리의 `CLAUDE.md` ↔ `AGENTS.md` 쌍 사이 bullet 중복은 카운트하지 않는다.** 그 둘은 sync 의도된 중복이므로 안티패턴이 아님.
- *placeholder 제외*: **placeholder 안내문은 dedup 대상에서 제외한다.** 다음 마커가 포함된 bullet은 사전 필터링 — `아직 없음`, `채워주세요`, `검토 필요`, `있다면 추가`, `비어 있다면`, `tbd`, `todo`, `예:`로 시작, `_(`로 시작. 템플릿이 의도적으로 여러 영역에 같은 안내문을 두는 것은 안티패턴이 아니다.
...
- **디렉토리 트리 (1번 이상)**: markdown 코드블록 안에 `├`/`└` 라인이 5개 이상이면 안티패턴. `ls`로 알 수 있는 정보를 가이드에 적으면 컨텍스트 토큰과 LLM 주의력을 낭비한다.
