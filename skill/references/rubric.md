# Agentic Project Init — 가이드 파일 품질 루브릭 v1.8

> **v1.8 변경**: T3 dedup에서 **placeholder 안내문 사전 제외**(예: "_(아직 없음)_", "있다면 추가", "검토 필요"). 템플릿이 의도적으로 여러 영역에 두는 안내문이 안티패턴으로 오인되던 문제 수정. 동시에 스킬은 명령어 가드를 root(공통)/영역(고유)으로 분리해 생성하도록 가이드라인 추가 — 진짜 중복은 스킬 단계에서 막는다.
>
> **v1.6 변경**: "@import 최소주의 원칙" 섹션 신설. 카파시 4원칙 시선에서 @import는 Both 모드 sync + 영역 진입 후 강결합 의존성에만 허용. `work-checklist.md` 신설 — 가이드 작업 전 점검할 18개 체크리스트.
>
> **v1.5 변경**: G1 안티패턴에 **markdown 디렉토리 트리** 검출 추가.
>
> **v1.4 변경**: `@./AGENTS.md` 같은 redirect 파일을 인식해 import 대상의 점수를 차용.
...
### T3. 규칙 중복 (1점) ⭐ v1.3 sync 쌍 제외, v1.8 placeholder 제외
...
- *v1.3 제외 룰*: **같은 디렉토리의 `CLAUDE.md` ↔ `AGENTS.md` 쌍 사이 bullet 중복은 카운트하지 않는다.** 그 둘은 sync 의도된 중복이므로 안티패턴이 아님.
- *v1.8 제외 룰*: **placeholder 안내문은 dedup 대상에서 제외한다.** 다음 마커가 포함된 bullet은 사전 필터링 — `아직 없음`, `채워주세요`, `검토 필요`, `있다면 추가`, `비어 있다면`, `tbd`, `todo`, `예:`로 시작, `_(`로 시작. 템플릿이 의도적으로 여러 영역에 같은 안내문을 두는 것은 안티패턴이 아니다.
