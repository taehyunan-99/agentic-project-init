# Agentic Project Init — 가이드 파일 품질 루브릭 v1.5

> **v1.5 변경**: G1 안티패턴에 **markdown 디렉토리 트리** 검출 추가 (코드블록 안의 `├`/`└` 라인 5개 이상이면 -3점). `ls`로 알 수 있는 정보를 가이드에 적는 패턴을 도구가 잡도록 함. 자연어 패턴은 기존대로 5번 이상 매칭 시 트리거, 트리 패턴은 1번이라도 매칭되면 트리거. 동시에 `root-map-template.md`의 "프로젝트 구조" 섹션 제거 — 스킬이 안티패턴을 기본으로 생성하지 않도록 수정.
>
