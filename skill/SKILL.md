...
<!-- 강결합 (자동 import — 영역 진입 후 함께 로드되어야 하는 SoT) -->
@../{other-area}/{파일명}

- 의존: [`{경로}`]({상대경로}) — 약결합 참고 (작업 시점에만)
- 피의존: ...
- 경계: ...
...
- **WHERE의 강결합/약결합 분리** (v1.6): 의존성을 두 종류로 표현한다.
...
- **첫 줄 목적 + Tradeoff 자리** (v1.7, 카파시 원칙 #16/#17): 가이드 헤딩 바로 아래에 두 placeholder 자리가 있다.
  - **첫 줄 목적**: 이 가이드가 "LLM이 자주 틀리는 무엇을 막는가"를 1문장. 영역 디렉토리/스택을 보고 추론 가능하면 초안 작성 ("frontend의 API contract 깨짐을 막는다" 등). 모호하면 placeholder 그대로 두어 사용자가 채울 자리 명시.
  - **Tradeoff**: 이 가이드/룰을 따르면 잃는 것 1문장. 일반적으로 사용자만 판단 가능하므로 기본 비워두고 주석 블록은 그대로 둔다.
- **명령어 가드의 root/영역 분리** (v1.8): 명령어 가드 중복은 영상의 T3 안티패턴(같은 규칙이 여러 가이드에 흩어짐)이다.
  - **root map의 "공통 명령어 가드"**: 모든 영역에 공통으로 적용되는 가드만 (예: `--no-verify` 금지, production DB 직접 쓰기 금지, `force push` 금지).
  - **영역 가이드의 "명령어 가드"**: 그 영역에서만 적용되는 가드만 (예: backend의 "async 핸들러에서 동기 IO 호출 금지", frontend의 "watch 모드로 CI 실행 금지").
  - 영역 초안 작성 시 root에 이미 있는 가드는 영역에 다시 적지 않는다. 사용자가 두 가이드를 함께 보면 컨텍스트가 가산된다.
...
- 동작: 프로젝트 내 모든 가이드를 v1.6 루브릭(`assets/rubric-schema.json`)으로 채점, 결과는 콘솔에만 출력.
...
- `assets/guide-audit-skill/` — 가이드 품질 채점 스킬 (v1.6 루브릭 기반)
- `assets/rubric-schema.json` — 자동 채점용 스키마
- `references/rubric.md` — 사람용 평가 기준서 (@import 최소주의 원칙 포함)
- `references/work-checklist.md` — 가이드 작업/검증 전 점검 18개 체크리스트 (영상 + 카파시 원칙 통합)
