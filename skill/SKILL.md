...
description: 임의 프로젝트 폴더를 AI 에이전틱 작업에 최적화된 구조로 초기화한다. root에 영역 map을 두고 각 영역마다 7섹션 작업 가이드(WHAT/CONTENTS/HOW/HOW NOT/WHERE/WHY/LEARNED CAUTIONS)를 생성하며, 동시에 `/learn` 슬래시 커맨드를 함께 설치해 작업 중 발견된 실수/주의사항을 영역별 가이드에 누적할 수 있게 한다. `/agentic-project-init` 슬래시 명령으로 직접 실행하는 스킬이며 자동 트리거하지 않는다. 인자에 따라 출력 파일명이 달라진다 — `claude`는 `CLAUDE.md`, `agents`는 `AGENTS.md`(Codex/Antigravity/Cursor 호환), `both`는 둘 다 + 양방향 sync 자동화(pre-commit hook + sync 스크립트). 인자 없이 호출하면 어떤 환경인지 사용자에게 묻고 대기한다. 이 스킬의 핵심은 (1) 토큰 효율을 위한 map 구조와 (2) 코드만 봐선 알 수 없는 함정·배경 지식을 영역별 가이드에 명시적으로 담는 것이다.
---
...
추가로 `/learn` 슬래시 커맨드를 함께 설치하여, 작업 중 발견되는 실수/주의사항을 해당 영역 가이드의 "⚠️ LEARNED CAUTIONS" 섹션에 누적해 나간다.
...
- **인자가 비어있으면**: 자동으로 모드를 가정하지 않는다. 다음 안내를 그대로 출력하고 사용자 응답을 대기한다.
  ...
  어떤 환경에서 진행할까요? 다음 중 하나를 선택해주세요:
    ...
    claude  — Claude Code용 (CLAUDE.md + .claude/commands/learn.md)
    agents  — Codex / Antigravity / Cursor용 (AGENTS.md + .agents/workflows/learn.md)
...
7섹션 모델(WHAT / CONTENTS / HOW / HOW NOT / WHERE / WHY / LEARNED CAUTIONS)을 따른다:
...
## 주의사항 학습 (`/learn`)
작업 중 실수가 발생하면 `/learn [메모]`로 해당 영역 {파일명}의 "⚠️ LEARNED CAUTIONS" 섹션에 누적한다.
```
