...
description: 임의 프로젝트 폴더를 AI 에이전틱 작업에 최적화된 구조로 초기화한다. root에 영역 map을 두고 각 영역마다 7섹션 작업 가이드(WHAT/포함 내용/HOW/HOW NOT/WHERE/WHY/학습된 주의사항)를 생성하며, 동시에 `/learn` 슬래시 커맨드를 함께 설치해 작업 중 발견된 실수/주의사항을 영역별 가이드에 누적할 수 있게 한다. `/agentic-project-init` 슬래시 명령으로 직접 실행하는 스킬이며 자동 트리거하지 않는다. 인자에 따라 출력 파일명이 달라진다 — 기본은 `CLAUDE.md`, `agents` 인자는 `AGENTS.md`, `both` 인자는 둘 다 + 양방향 sync 자동화(pre-commit hook + sync 스크립트). 이 스킬의 핵심은 (1) 토큰 효율을 위한 map 구조와 (2) 코드만 봐선 알 수 없는 함정·배경 지식을 영역별 가이드에 명시적으로 담는 것이다.
---
...
추가로 `/learn` 슬래시 커맨드를 함께 설치하여, 작업 중 발견되는 실수/주의사항을 해당 영역 가이드의 "⚠️ 학습된 주의사항" 섹션에 누적해 나간다.
...
- git 리포가 아니면: Both 모드는 pre-commit hook을 설치할 수 없으므로 "git 리포가 아닙니다. `git init` 후 다시 실행하거나, 기본/agents 모드로 전환하시겠습니까?" 라고 묻고 중단/전환 대기.
- 기본/agents 모드는 git 없이도 동작하므로 경고만 출력하고 계속.
...
5섹션 모델(WHAT / HOW / HOW NOT / WHERE / WHY)을 따른다:
...
{이 영역에서 "정상적인" 변경이 따라야 할 패턴/관례.
 예: 어떤 레이어를 거쳐야 하는가, 어떤 파일부터 손대는가, 테스트는 어디에 추가하는가.}

## 4. ⛔ HOW NOT — 시스템을 깨뜨리는 비명백한 함정 (중요)
{코드를 봐도 명확하지 않은, 그러나 어기면 시스템이 깨지는 규칙.
 "왜 안 되는지" 한 줄을 같이 쓴다 — 이유가 없으면 LLM이 룰을 무시할 가능성이 높다.}
- {예: τ/임계치/dt 값을 `config.py` 외부에서 하드코딩 — SoT는 `simulation/config.py`}
- ...

## 5. WHERE — 다른 모듈과의 의존성
{이 영역이 의존하는 곳 / 이 영역을 의존하는 곳 / 어댑터 경계.
 변경이 어디로 전파되는지 알면 에이전트가 영향 범위를 추정할 수 있다.}
- 의존: ...
- 피의존: ...
- 경계: ...
...
{도메인 지식, 과거 결정의 맥락, "왜 이렇게 짰는지" — 코드만 봐선 절대 모르는 것.
 예: 도메인 용어 정의, 과거 incident, 다른 길로 안 간 이유.}
...
- **HOW NOT과 WHY는 추측에 한계가 있다** — 일반적 함정(예: any 타입 회피, DB 직접 쓰기 금지)은 스택 기반으로 제시할 수 있지만, 진짜 가치 있는 항목(프로젝트 고유 함정, 도메인 결정 배경)은 사용자만 알 수 있다. 추론한 항목에는 "추정이므로 검토 필요" 표시를 붙이고, 추론할 수 없는 항목은 비워두되 placeholder 한 줄(예: "_(이 영역의 비명백한 함정이 있다면 채워주세요. `/learn`으로도 누적 가능합니다)_")을 남겨 사용자가 채울 자리를 명시한다.
...
## 주의사항 학습 (`/learn`)
작업 중 실수가 발생하면 `/learn [메모]`로 해당 영역 {파일명}의 "⚠️ 학습된 주의사항" 섹션에 누적한다.
```
...
- 작업 중 새로운 주의사항이 발견되면 `/learn [메모]`로 누적할 수 있다고 안내.
...
- `assets/area-guide-template.md` — 영역별 가이드 7섹션 템플릿
- `assets/learn-command.md` — `/learn` 슬래시 커맨드 정의
